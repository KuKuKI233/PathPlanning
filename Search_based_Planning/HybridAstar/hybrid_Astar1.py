...
from math import floor, pi, sin, cos, acos, atan2, sqrt
...
class ConstraintAStar:
    ...
    def __init__(self, s_start, direc_start,s_goal, direc_goal,heuristic_type):
        ...
        self.path=[]
        self.dubins_path_part=[] #dubins path
        self.dubins_path_length=0

        self.direction=dict() #车体方向
        self.direction[self.s_start]=self.direc_start


        #该节点被扩展的方式： 1 左转，0 直行，-1 右转；
        #或者说此时的前轮状态，左偏直行或右偏
        self.motion_mode=dict() 
        self.path_end=s_start
        self.steering_penalty=1.5 #非直行惩罚
        self.steering_change_penalty=1.8 #转向角改变惩罚
        ...
        self.car_node_steer_radius=1.5#节点扩展长度
        print("self.car_min_turnning_radius",self.car_min_turnning_radius)
        #计算节点扩展属性
        #扩展的子节点坐标与父节点坐标的偏差角度
        self.node_max_steer_angle=math.asin((0.5*self.car_node_steer_radius)/self.car_min_turnning_radius)
        #扩展的子节点(车体)方向与的父节点（车体）方向的偏差角度
        self.node_yaw_angle=2*self.node_max_steer_angle
        #两节点车头偏航角度
        # print(self.node_max_steer_angle/3.14)
        #设置最大dubins路径长度
        self.max_dubins_path_length=self.car_min_turnning_radius*9
    def searching(self):
        """
        A_star Searching.
        ...
        while self.OPEN:
            ...
            #是否到达目标点
            if(self.h2(s)<=self.max_dubins_path_length):
                if (self.use_dubins_curves_arrive_goal(s)):  # stop condition
                    ...
                    print("dubins_path_length:",self.dubins_path_length)
                    self.path_end=s
                    print("arrive goal",s,self.PARENT[s])
                    break
            ...
            for s_n in self.expanding_node(s):
                ...
                #检测s_n的状态，以此节省运行时间：
                s_detection=(floor(s_n[0]),floor(s_n[1]))
                if(s_detection in self.obs):
                    continue
                ...
                if new_cost < self.g[s_n]:  # conditions for updating Cost
                    ...
                    g=self.ComputeG(s_n,new_cost)
                    h2=self.h2(s_n)
                    h1=h2
                    # if(h2>=self.max_dubins_path_length):
                    #     h1=h2
                    # else:
                    ...
                    # if(times>1):
                    #     print(s_n,h2,self.motion_mode[s_n],self.motion_mode[self.PARENT[s_n]],file=file)
                    
                    heapq.heappush(self.OPEN, (self.ComputeF(s_n,h1,h2), s_n))
    ...
    """ 计算F """
    def ComputeF(self,s,h1,h2):
        F=1*max(h1,h2)+1*self.g[s] #self.g[s]的系数是为了加快速度
        return F

    def ComputeG(self,s,new_cost):
        #加入惩罚项
        if(self.PARENT[s]==self.s_start):
            g=new_cost
        else:
            if(self.motion_mode[s]!=self.motion_mode[self.PARENT[s]]):
                if(self.motion_mode[s]!=0):
                    g=new_cost*self.steering_change_penalty*self.steering_penalty
                else:
                    g=new_cost*self.steering_change_penalty
            else:
                if(self.motion_mode[s]!=0):
                    g=new_cost*self.steering_penalty
                else:
                    g=new_cost
        self.g[s]=g
        return self.g[s]
    ...
    def h1(self,s):
        ...
        return L[ind][0]*self.car_min_turnning_radius
    ...
    def use_dubins_curves_arrive_goal(self,s):
        L,ind=dubins_L(s,self.direction[s],self.s_goal,self.direction[self.s_goal],self.car_min_turnning_radius)
        #注意：此时path_length是归一化后的，即真实路径长度应该需要*self.car_min_turnning_radius
        path_length=L[ind][0]

        self.dubins_path_length=path_length*self.car_min_turnning_radius
        if self.dubins_path_length>self.max_dubins_path_length: 
    ...
    def extract_path(self, PARENT):
        ...
        self.path = [self.path_end]
        s = self.path_end
        # print("x",s,self.PARENT[s])

        while True:
            # time.sleep(0.1)
            # print(s)
            s = PARENT[s]
            # self.plot_path_point(s)
            self.path.append(s)
            # if(PARENT[s]!=self.s_start):
            # print(s,self.PARENT[s])
        ...
        for i in range(0,int(np.array(self.dubins_path_part).size/2)):
            self.path.insert(0,self.dubins_path_part[i])

        # self.path=self.delete_colilinear_points(self.path)       
        # self.path=self.delete_redundant_inflection_points(self.path)
        # print(list(self.path),file=file)
        return list(self.path)

    def plot_path_point(self,s):
        arrow_length=0.5#方向向量长度  
        ...
        plt.arrow(s[0],s[1],arrow_length * cos(self.direction[s]),arrow_length * sin(self.direction[s]),\
            width=0.01,length_includes_head=True,head_width=0.2,head_length=0.3,fc='b',ec='b')
        return True
...
def main():
    s_start = (10, 80)
    s_goal = (80, 10)
    direc_start=0*math.pi/2
    direc_goal=3*math.pi/2
    # s_start = (2, 2)
    # s_goal = (38, 2)
    ...
    plot.animation(path, visited, "hybrid A*")  # animation
